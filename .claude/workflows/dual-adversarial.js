...
if (!review) incompleteLegs.push('opus')
if (!codex) incompleteLegs.push('codex')
const gateStatus = incompleteLegs.length === 0 ? 'COMPLETE' : 'INCOMPLETE'
const crossModelDegraded = codexSource === 'claude-fallback'

log(`dual-adversarial #2: ${clean ? 'CLEAN' : `${blocking.length} blocking`} (opus verdict ${verdict}, codex ${codexSource}, legs: opus=${!!review} codex=${!!codex}, gateStatus=${gateStatus})`)

return { clean, verdict, gateStatus, incompleteLegs, codexSource, crossModelDegraded, findings, blocking: blocking.length }
