...
6. **Report.** The Workflow returns `{ clean, mode, gateStatus, incompleteLegs, verdict, codexSource, crossModelDegraded, verificationSameFamily, verificationDegraded, filtered, findings, backlog, violatedProperties, blocking, lenses, lensSource, probeDegraded }`. **ALWAYS surface `violatedProperties`** — each entry is a property a leg DISPROVED against the code (`property`, `evidence`, `by`). The gate fails closed if any VIOLATED property shipped no matching finding — checked PER PROPERTY — for the core legs, the team legs and the load-bearing auto-derived lenses (`security`/`amplify`/`frontend`). A BASE or EXPLICIT lens (`codeQuality`/`completeness`) that records a VIOLATED property without a matching finding is enumerated in `incompleteLegs` as `lens:<key>:violated-unreported` but does NOT block, so `clean` can be true with an unpaired violation from those lenses — check `incompleteLegs` and `violatedProperties` before calling a run covered; report them alongside so the human sees the leg's own evidence, not just the severity the leg chose. **`findings` is Critical/Important ONLY — every Minor is in `backlog`.** ALWAYS surface `backlog`: list each entry with its severity, `by:` leg, file and summary, stated as "reported, not fixed in this loop". These are real findings that were deliberately routed out of the fix loop, not discarded — omitting them from the report is the one way this routing turns into silent deletion. Note that a pre-existing (not-introduced-by-this-diff) defect is reported at its true severity, prefixed `pre-existing:` — never suppressed, and never downgraded to land it in `backlog`. Scope decides ROUTING only: a **pre-existing Critical still BLOCKS**, while a pre-existing Important joins the Minors in `backlog` (reported, fixed later). When reporting the backlog, say how many of its entries are pre-existing rather than newly introduced — that count is the difference between "this change added debt" and "this change surfaced debt". **Lead with `clean` — it is the authoritative merge signal. Do NOT lead with `verdict`:** `verdict` reflects the Opus leg ONLY, so it can read `PASS` while the Codex leg surfaced a Critical (in that case `clean` is `false` and `blocking > 0`). State the `mode` used (`standard` or `team`). If `filtered` is non-empty (Team mode only), report how many false-positive findings the Team verification dropped AND list each dropped finding with its skeptic `reasons` — a dropped finding is the one audit trail the human has at the merge decision, so surface it, never silently omit it. If `verificationSameFamily === true` (Team mode, the verification pass ran), add the caveat that the skeptics were same-family Opus (not a true cross-model verification), exactly as you would for `crossModelDegraded`. If `verificationDegraded === true` (Team mode — a skeptic could NOT run), add the STRONGER caveat that a blocking finding could not be adversarially verified at all (a real verification gap, not merely same-family) and re-run the gate. If `probeDegraded === true` (auto-lens runs only), add the caveat that the changed-files probe failed or returned a malformed result, so the run widened to the FULL auto-selectable lens set (`lenses`) as a degraded fallback rather than a precise per-path derivation (`lensSource` is `auto`) — the extra lens coverage is defensive breadth, not a signal that those specific domains changed. Route on `clean` first, then `gateStatus`:
   - **`gateStatus === 'INCOMPLETE'`** (a core leg failed — see `incompleteLegs`) → the gate did NOT complete. Tell Santiago the gate is incomplete and **re-run the Workflow**. Do NOT offer `/iago-prfix` — a failed leg is not a fixable code defect, and `/iago-prfix` will not run the missing leg. An `incompleteLegs` entry ending in `:no-proof` (`opus-review:no-proof`, `codex:no-proof`, `team:data:no-proof`) is the same class of failure with a different shape: the leg *returned*, but its proof of work was empty or unevidenced, so it reviewed nothing it can be held to. Report it as "the leg produced nothing — unreviewed, not clean" and re-run. An entry ending in `:violated-unreported` is the inverse: the leg recorded a VIOLATED property and then returned an empty `findings` array, so it disproved something and reported nothing actionable — also a re-run, and name the violated property when you report it.
...
- **Minor findings are reported, not fixed in-loop.** Minors are partitioned into `backlog` and never enter a fix round; `findings` and `blocking` carry Critical/Important only. `clean` is computed exactly as before. A defect this diff did not introduce is reported at its TRUE severity, never softened. Routing then reads TWO axes: severity AND scope. A pre-existing Critical still BLOCKS; a pre-existing Important routes to `backlog` alongside the Minors, but ONLY when the leg supplied `preExistingEvidence` (the base-commit proof) — an unevidenced scope claim is ignored and the finding blocks. Everything newly introduced routes by severity alone, never by scope, and the gate never suppresses or downgrades a finding at emission time (see `.claude/rules/execution-pipeline.md`). The Report step must surface `backlog`, or the routing becomes silent deletion.
