...
**Stage 2 — Quality Review (review-quality capability).** Only reached if Stage 1 produces zero Critical findings. Assess code quality across performance, maintainability, TypeScript strictness, React/DynamoDB/Lambda conventions, and security. Apply the security capability again for a full OWASP + AWS pass. Produce a consolidated finding list with severity ratings and a final verdict: approve or request-changes.
