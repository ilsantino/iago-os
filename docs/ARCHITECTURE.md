...
- **`.iago/learnings/patterns.md`** accumulates recurring review findings — anti-patterns flagged by `review-single` and `review-full` profiles across sessions
...
| Code-writing profiles (executor-based) | Opus | fullstack, frontend, backend, debug, e2e, security-audit |
| Review/analysis profiles (analyst/operator-based) | Sonnet | review-single, review-full, research, infra, schema, content |
| Mechanical tasks | Haiku | Formatting, simple lookups, PR review tagging |
...
**Model routing** is hardcoded per profile in frontmatter (`model: opus` or `model: sonnet`). The `security-audit` profile always uses Opus regardless of other settings. Codex adversarial review uses GPT-5.4 when available; SDD and code-review skills automatically fall back to a Claude adversarial session if the Codex CLI is not installed.
