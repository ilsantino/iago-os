...
5. **Operate multiple agents simultaneously** across shapes, coordinating without colliding. Subagent spawn semantics (cortextOS deeper-adoption) link parent-child handles; cost rollup flows up; automatic shutdown when parent exits. The supervisor agent dispatches; specialists claim and execute; results merge through ticket-style coordination.
   - *Amendment 2026-05-29:* The 'supervisor agent' is now a named **chief role** — `role:"chief"` config flag granting the file-bus producer-capability (write peer-addressed tasks). NOT a new `AgentRuntime` shape, NOT a broker. See P1 feature 'Supervisor/chief role + inter-agent comms channel' below.
...
- **NOT a message-passing protocol.** File-bus is the protocol. No pub/sub, no agent-to-agent direct messaging, no ACP equivalent. Inter-agent coordination is `tasks/{pending,claimed,resolved}/` only.
  - *Amendment 2026-05-29 (envelope hardened per ADR 2026-05-30):* Refined — agents MAY be **file-bus producers**. A `role:"chief"` agent may write the canonical typed envelope `{v, kind:"agent-message"|"task-assignment", from, to, body, threadId, seq, needsApproval, quality_signal, createdAt}` into `tasks/pending/<toAgentId>__<uuid>.json` (mirrors cron `fire()`), claimed via the existing O_EXCL primitive, logged to `comms/<date>.ndjson`. `from` is server-stamped (anti-spoof), `seq` is the monotonic per-thread ordering/replay-dedupe key, and `quality_signal` carries the signed-chief-as-blocker authority — `v`/`seq`/`quality_signal` are load-bearing; do not implement the reduced shape (canonical: ADR `.iago/decisions/2026-05-30-per-agent-bots-and-chief-tier.md` + `feature-v2-agent-comms-channel/CONTEXT.md`; see P1 item 5a). The ban remains on a SEPARATE transport (broker/pub-sub/ACP). This realizes Mission #5 ('supervisor dispatches; specialists claim'). Trail: `.iago/research/2026-05-29-cortextos-comms-gap-analysis.md`.
...
5a. **Supervisor/chief role + inter-agent comms channel (Amendment 2026-05-29; envelope + authority hardened per ADR 2026-05-30).** `role: "chief" | "worker"` (default worker) in agent config. A `produceToPeer()` producer in a new `runtime/daemon/peer-bus.ts` mirrors `cron-scheduler.ts fire()` (tmp + atomicRenameStaleDest into `tasks/pending/<toAgentId>__<uuid>.json` — the `uuid` + `seq` below are MANDATORY deviations from `fire()`'s whole-second `__<unix>.json` filename, which silently overwrites multiple same-second sends); a producer-capability gate rejects non-chief callers and validates `from`/`to` (same rigor as `validateAgentId`). **Canonical envelope (single source of truth: ADR `.iago/decisions/2026-05-30-per-agent-bots-and-chief-tier.md` + `feature-v2-agent-comms-channel/CONTEXT.md` — do not change the field set without an ADR):** `{v, kind:"agent-message"|"task-assignment", from, to, body, threadId, seq, needsApproval, quality_signal, createdAt}`. `from` is **server-stamped by the daemon** (the producing process's identity, NOT a caller-supplied value — anti-spoof). `seq` is a monotonic per-thread sequence number (the file-bus has no ordering primitive — `seq` is what makes ordering + replay-dedupe possible). `quality_signal` carries the **chief authority signature** (structural/HMAC), which is a **BLOCKER** — authority is NOT the self-assertable `role:"chief"` config flag. Status is directory-derived. Append-only `comms/<date>.ndjson` log. Spend/prod-touching peer sends gate through the existing `approval-bus.ts` handshake. Per-agent inbound quota (resolves Open-Q11 cross-shape fairness). **Acceptance:** chief can enqueue; worker cannot (gate rejects); `from` is server-stamped (a peer cannot spoof the sender); structural/HMAC chief signing verified on the consumer side BEFORE any peer dispatch is honored (a missing/invalid signature is rejected); `seq` present and monotonic per thread, enabling ordering + replay-dedupe across multi-step A→B→C chains; cross-runtime codex→claude delivered via `AgentRuntime.send()`; approval-gated path; comms tab renders the NDJSON. NO broker, NO Postgres, NO new shape.

   - *Amendment 2026-05-29 (LangChain promotion):* Shape 2 (`anthropic-sdk.ts` + `openai-sdk.ts`, item 5 above) ships a **runnable LangGraph host-process example agent** that registers in the daemon and claims a file-bus task — not just an adapter shell. This is Santiago's explicit 'i want langchain' deliverable. Decide Open-Q7 (LangGraph checkpointer vs `session.jsonl` replay) before this executes.
...
8. **Full Next.js dashboard.** Full cortextOS port via IPC server. Agent list across all 5 shapes, per-shape filters, current state, recent activity, token spend per agent / project / model / **shape**. Streamlit fallback dropped 2026-05-15.
   - *Amendment 2026-05-29:* Add three read-only tabs backed by new IPC methods `list-tasks` (reads the three task dirs) and `list-crons` (reads crons.json): **Comms** (tail `comms/<date>.ndjson`), **Board** (kanban over `tasks/{pending,claimed,resolved}/`, In-Progress cards show owner agentId + shape badge), **Workflows** (cron registrations + last-fire telemetry). Honor existing H1 buffer + H2 idle caps on dashboard endpoints. The cortextOS 'comms tab' is now backed by real data (the comms channel). +2-3d on Phase 6.
...
2. **Telegram bot strategy.** ✅ DECIDED 2026-05-13 — one bot, per-agent file-bus tagging.
   - *Revisit 2026-05-29:* Lock KEPT. Per-agent identity via sticky `/agent <id>` default-target (ship now) + optional forum-topics on the single bot (gated — reopens the PR45 private-chat-only gate at `bot.ts:306`). Per-agent bot tokens stay rejected. Trail: `.iago/research/2026-05-29-cortextos-comms-gap-analysis.md` (the planned `2026-05-29-per-agent-identity-and-chief-tier.md` decision file was never written — this stance was reversed the next day; see *Superseded 2026-05-30* below).
   - *Superseded 2026-05-30 (Santiago LOCKED):* Reversed to **per-agent bots for standing agents + one chief/orchestrator bot for ephemeral workers & broadcast.** The one-bot HTTP-409 premise was wrong (409 is per-token; N tokens poll independently); cortextOS itself uses per-agent bots (README 'credentials for each agent'); per-agent bots are private DMs so the private-chat gate stays closed (forum-topics dropped). Cost: N one-time BotFather registrations + N tokens via per-agent `LoadCredential=`. ADR: `.iago/decisions/2026-05-30-per-agent-bots-and-chief-tier.md`.
...
11. **Cross-shape task fairness.** Different polling cadences across shapes (Daemon polls 10x/sec vs Webhook on event arrival) create starvation risk. O_EXCL prevents double-claims but not starvation. Per-shape quotas, per-agent quotas, or weighted-fair-queueing? Defer to Phase 3 when multiple shapes coexist; flag for monitoring as Phase-3 acceptance gate.
   - *Amendment 2026-05-29:* Adding agents as task PRODUCERS (chief dispatch) makes per-agent **inbound quota** required, not just monitored — a chatty chief could flood a worker's queue. Quota lands in the P1 5a comms-channel feature. Reuse `max_concurrent_children` (default 3) + the per-agent 200-file pending-queue migration trigger.
