...
- **NOT a message-passing protocol.** File-bus is the protocol. No pub/sub, no agent-to-agent direct messaging, no ACP equivalent. Inter-agent coordination is `tasks/{pending,claimed,resolved}/` only.
  - *Amendment 2026-05-29:* Refined — agents MAY be **file-bus producers**. A `role:"chief"` agent may write a typed `{ kind:"agent-message"|"task-assignment", from, to, body, threadId, needsApproval }` envelope into `tasks/pending/<toAgentId>__<uuid>.json` (mirrors cron `fire()`), claimed via the existing O_EXCL primitive, logged to `comms/<date>.ndjson`. The ban remains on a SEPARATE transport (broker/pub-sub/ACP). This realizes Mission #5 ('supervisor dispatches; specialists claim'). Trail: `.iago/research/2026-05-29-cortextos-comms-gap-analysis.md`.
...
5a. **Supervisor/chief role + inter-agent comms channel (Amendment 2026-05-29).** `role: "chief" | "worker"` (default worker) in agent config. A `produceToPeer()` producer in a new `runtime/daemon/peer-bus.ts` mirrors `cron-scheduler.ts fire()` (tmp + atomicRenameStaleDest into `tasks/pending/`); a producer-capability gate rejects non-chief callers and validates `from`/`to` (same rigor as `validateAgentId`). Typed envelope `{kind:"agent-message"|"task-assignment", from, to, body, threadId, needsApproval, createdAt}`; status is directory-derived. Append-only `comms/<date>.ndjson` log. Spend/prod-touching peer sends gate through the existing `approval-bus.ts` handshake. Per-agent inbound quota (resolves Open-Q11 cross-shape fairness). **Acceptance:** chief can enqueue; worker cannot (gate rejects); cross-runtime codex→claude delivered via `AgentRuntime.send()`; approval-gated path; replay-dedupe; comms tab renders the NDJSON. NO broker, NO Postgres, NO new shape.
...
2. **Telegram bot strategy.** ✅ DECIDED 2026-05-13 — one bot, per-agent file-bus tagging.
   - *Revisit 2026-05-29:* Lock KEPT. Per-agent identity via sticky `/agent <id>` default-target (ship now) + optional forum-topics on the single bot (gated — reopens the PR45 private-chat-only gate at `bot.ts:306`). Per-agent bot tokens stay rejected. Trail: `.iago/decisions/2026-05-29-per-agent-identity-and-chief-tier.md`.
