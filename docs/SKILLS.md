...
**Arguments:** `--discuss` clarifies first. `--research` dispatches researcher first. `--verify` verifies after. Runs the full 8-stage pipeline (stress test → implement → build gate → review → codex → codex fix → PR → summary) with async GitHub Action fix loop.
...
**Arguments:**
- `--pipeline` — run each task through `scripts/execute-pipeline.sh` for full 8-stage review isolation (stress test → implement → build gate → review → codex → codex fix → PR → summary). Recommended for production code.
...
**Profiles:** Matching profile per task (fullstack/frontend/backend), `review-single` or `review-full`. Codex adversarial review mandatory; falls back to Claude adversarial if Codex CLI unavailable.

**Example:**
```
> /subagent-driven-development .iago/plans/phase-1-plan-01.md
# Task 1 → matching profile → review → codex → Task 2 → ...

> /subagent-driven-development .iago/plans/plan.md --pipeline
# Each task → execute-pipeline.sh (8-stage, separate sessions)
```
