...
- [x] If there's a workaround, the upstream issue is filed AND the workaround documents the issue link (resolved by `runtime/daemon/state-paths.md` audit per Plan feature-phase-1-deferred-hardening/02 — the leaky single-function `atomicRename` API is now split into strict `atomicRename` (link+unlink, fail-on-EEXIST cross-platform) and destructive `atomicRenameStaleDest` (Windows EEXIST recovery for stale-dest callers); PR #45's raw `fsp.rename` CLAIM workaround in `approval-bus.ts` migrated to the new strict `atomicRename` since `link+unlink` provides identical fail-on-EEXIST semantics on both Windows and POSIX without the silent-overwrite hazard of raw POSIX `rename(2)`)
