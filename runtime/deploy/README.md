...
| `cutover.sh` | Full FAST cutover (stop OpenClaw → archive → install unit → enable → start) | Santiago's local box (drives the VPS over Tailscale SSH) | All Plan 01a + 02a artifacts present | N (state-changing) | _(landed via Plan 03a)_ |
| `rollback.sh` | Rollback (stop v2 → restart OpenClaw from archive) | Santiago's local box (drives the VPS over Tailscale SSH) | Archive present, OpenClaw archive intact | N (state-changing) | _(landed via Plan 03a)_ |
...
0. **Provision the box** — `IAGO_BOOTSTRAP_CONFIRM=YES bash runtime/deploy/bootstrap-vps.sh`. Creates the `iago` user, the state root and its subdirs, the log dir, the credstore, and the `/opt/iago-os` checkout, then runs `npm ci` + `npm run build` on the VPS. Safe to run days ahead — it is additive and does not touch a running OpenClaw.
   ...
   > **Correction (2026-08-19).** The § 2 table previously listed `cutover.sh` and `rollback.sh` as running on the **VPS**. They run on **Santiago's local box** and drive the VPS over Tailscale SSH — 40 `tailscale ssh` calls, a locally-rendered unit file piped to the remote, pre-flight checks that read the local repo checkout, and a T+05 step that invokes `provision-credentials.sh` locally. That last point is load-bearing: `provision-credentials.sh` requires the 1Password CLI, and the ADR states plainly that **the 1Password CLI never runs on the VPS**. `op` therefore belongs on the operator's machine only; nothing about this sequence needs it installed on the VPS.
