...
| Telegram API outage | Daemon `sendAgentNotification` returns `{ ok: false }` | The daemon claims the envelope first (at-most-once — no duplicate-send storm), then retries the send a BOUNDED number of times (`TELEGRAM_SEND_RETRY_BACKOFF_MS`) so a single transient 429/network blip is absorbed in-handler; only if the bounded retry is exhausted does it emit `pr-triage-telegram-send-failed` and drop this run's summary (recorded as telemetry, surfaced next daily run). The next-day run still fires |
