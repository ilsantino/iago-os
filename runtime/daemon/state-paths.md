...
| `runtime/telegram/approval-bus.ts:769` | `recoverStrandedApprovals` inflight→pending rename | `inflightP` (= `inflight/<id>.json`) | `pendingP` (= `pending/<id>.json`) | At boot, rename an inflight-only orphan back to pending so the bot can re-broadcast it and the next resolveApproval call goes through the normal CLAIM path. |
...
| `approval-bus.ts:769` recoverStrandedApprovals inflight→pending | **stale-dest** | At boot recovery, `pendingP` is confirmed absent by the `pathExists(pendingP)` guard above (line 758); the dual-presence branch at line 759 handles the case where it exists. Boot-time recovery is single-process by design. Destructive replacement is safe and is the correct intent — restore the orphan inflight to pending for re-broadcast. | `atomicRenameStaleDest` |
...
- **Out-of-scope raw `fsp.rename` at `file-bus.ts:252` (task CLAIM path).** This call was never an `atomicRename` caller and is therefore outside the grep-based inventory above. On POSIX, `rename(2)` silently overwrites `claimed/<taskId>.json` if it pre-exists; on Windows it throws EEXIST and the handler returns `already-claimed`. The cross-platform asymmetry mirrors the original approval-bus hazard. In practice it is unreachable (taskIds are UUIDs; a pre-existing claimed file would require taskId reuse, which violates the contract at `file-bus.ts:8-9`), but should be addressed in a follow-on audit pass alongside any multi-process extension.
