...
- **DEVIATION AS BUILT (recorded, not silently taken):** the last clause is INVERTED in the shipped PREAMBLE. As built it reads "SCOPE, NEVER SUPPRESSION: … Routing is by SEVERITY, not by scope: Critical/Important go to the fix loop and Minor goes to the backlog, for pre-existing and newly-introduced defects alike. Do NOT downgrade a pre-existing Critical to Minor to 'route it to the backlog'." Reason: scope-based routing licenses exactly the suppression the standing rule forbids (`.claude/rules/execution-pipeline.md`: reviews never dismiss findings as "acceptable"/"carry-over") and contradicts Task 7's own "routed, never suppressed at emission". A pre-existing Critical routed to the backlog by SCOPE would never block. The as-built text is pinned by the test "the scope clause routes by SEVERITY (a pre-existing Critical is not 'backlog-routed')" — do NOT restore the plan's original wording.
