...
- **I2 — Fixture `expected-events.json` event-count semantics.** Per-24h counts use the `expected_count_per_24h` field. The `heartbeat` kind is `expected_count_per_24h: 0` with `floor_applies: false` — the daemon emits it sparsely (state changes / liveness probes), NOT once per 60s tick, so 05b applies **no** count floor to it. 05b floors only entries with a positive `expected_count_per_24h` and `floor_applies` not `false`. (The original Plan 05 `heartbeat-tick @ 1440 / 95%-floor` sketch was corrected during this plan's dual-adversarial review; liveness is proven by journalctl + pgrep, not a heartbeat line count.)
...
3. expected-events.json uses `expected_count_per_24h` + `floor_applies` fields; `heartbeat` is count 0 / `floor_applies: false` — 05b applies no heartbeat floor (I2 fix).
