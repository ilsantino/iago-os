...
- **I2 — Fixture `expected-events.json` event-count semantics (presence-based).** `expected_count_per_24h` is a documented cadence hint, NOT a test floor. 05b test 7 applies **no** numeric count floor to any kind — it presence-checks (≥5 of ≥10 kinds appear) and requires the two `required` startup kinds. Every non-`required` kind carries `floor_applies: false` because each is legitimately 0 on a quiet/sparse window (`cron-*`/`agent-*`/`task-*` on a zero-PR day; `heartbeat` sparse; `telegram-message-sent` synthetic, count 0). (The original Plan 05 `heartbeat-tick @ 1440 / 95%-floor` sketch was corrected during this plan's dual-adversarial review; liveness is proven by journalctl + pgrep, not a telemetry line count.)
