...
- **I2 — `expected-events.json` event-count assertions honor `floor_applies`.** The shipped fixture marks `heartbeat` as `criticality: optional`, `expected_count_per_24h: 0`, `floor_applies: false` — the real daemon emits `heartbeat` sparsely (state changes / liveness probes), NOT one line per 60s tick, so there is **no** 1440/day floor. Test 7 MUST NOT apply a 95%-of-1440 floor to heartbeat (it would false-FAIL every real capture). Apply a per-24h count floor only to entries with a positive `expected_count_per_24h` and `floor_applies` not `false`. Daemon liveness over a 24h window is proven by journalctl + pgrep (tests 4, 8), not by a heartbeat line count. (Supersedes the original Plan 05 `heartbeat-tick @ 1440` sketch — corrected during the 05a dual-adversarial review and locked by `runtime/integration/phase-2-evidence-template.test.ts`.)
...
- **Edge cases:** C1 (nondisruptive subset) + C2 (sentinel replacement) + C3 (prose vs fenced) cover the brittle paths. I2 (honor `floor_applies`; no heartbeat 1440 floor) covers event-count assertions.
...
5. Event-count assertions honor `floor_applies`/`criticality`; NO 1440 heartbeat floor (I2 fix).
