...
- **I1 — `systemd-analyze security` score regex.** Spec § 1 verification says "exposure level ≤2.0". The number is `0.0 ↔ 10.0` where LOWER is BETTER. Both check-evidence.mjs `--strict` mode AND phase-2-vps.test.ts test 2 use the same regex (`/Overall exposure level [^:]*:\s*(\d+\.\d+)\s+(UNSAFE|DANGEROUS|EXPOSED|MEDIUM|OK|SAFE)/m`). Drift between the two would mean a fixture parses but the live test doesn't (or vice versa). Both share a single helper function in check-evidence.mjs that phase-2-vps.test.ts imports — DRY mitigation.
- **I2 — `expected-events.json` assertions are presence-based, never count-floored.** The shipped fixture marks all 9 non-`required` kinds with `floor_applies: false` because each is legitimately 0 on a quiet/sparse window: `cron-*`/`agent-*`/`task-*` are 0 on a zero-PR day (cron-fired and cron-skipped are mutually exclusive per tick), `heartbeat` is emitted sparsely (NOT 1440/day), and `telegram-message-sent` is synthetic (count 0, no emitter — success is inferred from the resolved `pr-triage-send__*.json` envelope + the absence of `pr-triage-telegram-send-failed`). Test 7 MUST NOT apply any numeric per-24h count floor (it would false-FAIL a healthy capture); it presence-checks kinds and requires only the two `criticality: required` startup kinds (`daemon-start`, `cred-bootstrap-loaded`) to be present in a post-cutover capture — read the required set from the fixture's `criticality` field, never hard-code a kind count. Daemon liveness over a 24h window is proven by journalctl + pgrep (tests 4, 8), not by a telemetry line count. (Supersedes the original Plan 05 `heartbeat-tick @ 1440 / 95%-floor` sketch — corrected during the 05a dual-adversarial review and locked by `runtime/integration/phase-2-evidence-template.test.ts`.)
...
- **Edge cases:** C1 (nondisruptive subset) + C2 (sentinel replacement) + C3 (prose vs fenced) cover the brittle paths. I2 (presence-based telemetry check; no count floors; all 9 non-`required` kinds `floor_applies: false`) covers event-count assertions.
...
5. Telemetry test 7 is presence-based — NO count floors; `required` kinds must be present, `expected`/`optional` are bonus (I2 fix).
