...
| P6 enforcement | **DONE** 2026-08-19 — `scripts/organize/sweep.py` + `test-sweep.py`. Registered with Task Scheduler as "iaGO File Sweep", daily 09:00. Acts on high/medium-confidence renames and entity routing; reports low-confidence names, aged installers and per-zone drift. Never deletes. **Fixed 2026-08-20** — schtasks had registered it as battery-refusing, so its first unattended run declined silently and wrote no report; power flags corrected, verified running on battery. |
