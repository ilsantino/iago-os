...
- **action:** `git mv` all three into `.iago/plans/_archive/2026-05-mwp-restructure/` with a `README.md` stating they are superseded by `feature-doc-standard/`, that docs/01 and docs/02 shipped as PRs #77 and #79 while the rest never executed, and that they must not be run without re-stress-testing. The archive convention calls for a roadmap pointer, but `.iago/ROADMAP.md` has no row for this work — add one, or drop the pointer framing and say why in the archive README.
  **The README must flag `feature-mwp-restructure-code/01-iago-physical-split.md` as carry-forward, not dead history.** Its §"Stress-test BLOCK fix (C1+C3)" holds the atomic-Bash-call fix for the hooks move that plan 02 Task 5 depends on — a fix this feature initially failed to inherit and re-discovered only in stress. An archive that reads as "superseded, ignore" invites that mistake again. Note in the same README the two corrections found on re-reading it: `.claude/settings.json` is **not** config-protected (only `biome.json`, `tsconfig.json`, `.gitignore`, `Dockerfile` are), and there are 8 hook `.mjs`, not ≥9.
