...
_**Severity escalated to CRITICAL (2026-06-02).** PR #84's pass#2 dual-adversarial gate (Opus ∥ Codex GPT-5.5) re-rated this finding **Critical**: the data-loss mechanism is a daemon crash in the window between `claimTask` resolving the envelope (`pending/`→`resolved/`) and the Telegram send completing — including during the bounded 250ms/1000ms retry backoff. The resolved envelope is never re-polled, the in-memory dead-letter timer does not survive the restart, and `bootRecovery`/`recoverStrandedApprovals` reconcile neither — so the daily summary is silently lost with no durable pending work to recover. Still deferred here per Santiago's 2026-06-02 call: bounded to a missed daily notification on the **UNDEPLOYED** daemon (OpenClaw still runs), design-intrinsic at-most-once cost; **must land before daemon cutover.** Fix is the same as above + an idempotency key so a re-armed dispatch cannot double-send. Also folds in gate finding #2: the cron `maxConcurrent` slot is released at prompt-handoff (`claimTask`→`task-resolved` decrements `runningCount`), NOT at run completion, so a slow/hung run lets the next cron fire dispatch a second prompt that overwrites the single-key result timer and emits a stale/duplicate envelope — close it by holding the slot until the result envelope is processed or a durable timeout fires._
