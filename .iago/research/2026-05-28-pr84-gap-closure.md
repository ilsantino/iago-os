...
## Post-impl dual-adversarial rounds (Opus 4.8 + Codex GPT-5.5)

The pipeline's own Codex stage no-op'd (it diffed a commit-range against an unmoved HEAD because `--no-pr` commits *after* the codex stage → empty diff). The independent post-impl pass on the committed diff caught real findings across four Codex rounds:

- **Round 1 (commit `ca51f9e`)** — Codex H1: ndjsonAlert branch was un-scoped (any agent/any kind on the shared `tasks/pending` bus could bypass dispatch). Codex H2: secret gate keyed on self-declared `agentConfig.org`. Fixed: `PR_TRIAGE_ALERT_KINDS` + `agentId==="pr-triage"` + prompt-less gating; `CRON_AGENT_SECRET_TRUSTED_AGENTS` (daemon-owned, agentId-keyed). Negative tests Case 10/11.
- **Round 2 (commit `bdd5887`)** — Codex H (at-rest): persisted config written world-readable (0644/0755) now that it carries daemon secrets → `persistAgentConfig` writes 0600 + chmod, `ensureStateDirsSync` 0700. Codex Medium (durability): `emit()` swallowed write failures → now returns boolean; ndjsonAlert branch claims only when durably recorded. Tests Case 12/13.

### Deferred to Phase 2 (explicit, CEO-approved Option A — harden perms now, defer encryption/recompose)
Codex round 4 re-raised two items that belong to the planned Phase-2 systemd credential work, NOT this PR:

1. **fsync-backed durable telemetry for gated alerts** — `emit()` returns true after `appendFile` resolves (OS buffer, not stable storage). A host power-loss in the flush window could lose one `pr-triage-telegram-send-failed` record after the task was resolved. Phase-2 durable-telemetry layer (open+append+`datasync`+dir-fsync for gated events). Low practical risk: rare daily-cron-on-Telegram-outage path; daemon not yet deployed.
2. **Recompose daemon secrets from `process.env` on boot recovery (don't persist raw)** — persisting the secret snapshot means a post-rotation boot-recovery could spawn the cron handle with stale tokens; combined with same-org duplicate-agentId tolerance + `findHandleForAgent` first-match, a stale recovered handle could remain the dispatch target. This is the credstore recompose-on-spawn design deferred to Phase-2 systemd `LoadCredential=`. Also enforce one-live-handle-per-cron-agentId at that time.

Both are tracked here so Phase-2 (daemon cutover hardening) closes them; neither is a blocker for the not-yet-deployed daemon.
