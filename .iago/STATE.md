...
|------|------|-------------|--------|
| 2026-05-19 | spec | Strategy sync: three-invocation-modes ADR + sentry-integration spec. Clarifies that pipeline (Mode 1) is one of three first-class agent invocation modes alongside direct/Telegram (Mode 2) and headless/scheduled (Mode 3) — all three coexist post-cutover. Sentry integration mapped across 4 layers spanning Phases 3, 9, and 10 (Layer A daemon obs, Layer B per-client apps, Layer C Sentry-MCP-setup-required, Layer D auto-fix dispatch with iago-os-self loop FIRST at Phase 10). | [#71](https://github.com/ilsantino/iago-os/pull/71) → `9f45cf5` |
