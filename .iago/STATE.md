...
> **Phase:** v2-phase-2-vps-bootstrap | **Status:** Phase 2 execution in flight. Phase 2 plan PRs merged: #60 (02a), #61 (07a), #62 (01a), #63 (02b), #64 (07b), #65 (01b), #67 (04a), #68 (03a), #72 (03b). Remaining: 04b PR-triage wiring/test, 05a/05b evidence template+checker+e2e, 06 SIGHUP credential reload. Cutover Sunday 2026-05-25 20:00 US/Mexico (5 days out).
> **Tag:** v0.1.0 | **Updated:** 2026-05-20

## Active

| Date | Mode | Description | Commit |
|------|------|-------------|--------|
| 2026-05-20 | merge | PR #72 — cutover and rollback runbooks for VPS bootstrap (Plan 03b). Pipeline halted at review-fix boundary on out-of-scope IMPLEMENT-session drift (gmail-bot-filters.gs Apps Script, unrelated to 03b — orphan preserved at `.iago/state/orphans/`); manually finished via codex-companion adversarial-review as stage-4 substitute. Dual aggressive adversarial (Opus 4.7 subagent + Codex GPT-5.5) found 24 findings beyond async @claude loop's surface coverage — 3 Critical + 5 High addressed via 2 sequential fix-session dispatches (commits `ba00259` + `8fa1448`). Async loop ran 5 rounds hitting max-rounds safety. H1 (T+25/T+30 structural boundary with 4-item acceptance gate) shipped in `8fa1448`. Tier 3 (8 Important + 5 Medium + 1 Minor) deferred non-blocking. Full audit at `.iago/reviews/2026-05-20-dual-pr72-aggressive/`. | [#72](https://github.com/ilsantino/iago-os/pull/72) → `b09479c` |
| 2026-05-20 | spec | PostHog + Sentry observability split + SQLite-as-6th-layer memory verdict. Triple-agent research (PostHog vs Sentry, memory patterns, current planning state) → ADR `.iago/decisions/2026-05-20-posthog-sentry-split-and-memory.md`. Sentry spec rewritten from 4-layer Sentry-only to 5-layer split: Sentry owns Layer A (daemon errors, real-time hooks) + Layer D default (auto-fix dispatch, re-evaluated at Phase 10 impl); PostHog owns Layer B (per-client app errors + analytics — free for 3+ users where Sentry caps at 1) + Layer E NEW (LLM telemetry via Claude Code plugin, zero VPS infra, installable today); Layer C dual-MCP (Sentry MCP for trace context, PostHog MCP for cost queries). Memory: 5-layer carries forward unchanged + SQLite (already planned in v2 vision §§ 132, 472) formally named as the 6th layer for per-agent session resumption / cost ledger / event-dedupe. Postgres + data warehouse + Zep all deferred with explicit triggers. Confidence 90% obs split, 95% memory. NO retroactive changes to locked Plan 01a/01b — credential provisioning queued for Phase 3 PRs. | (this commit) |
| 2026-05-19 | merge | PR #71 — strategy sync (three-invocation-modes ADR + sentry-integration spec). Async @claude loop ran 2 fix rounds; dual aggressive adversarial (Opus 4.7 subagent + Codex GPT-5.5 via codex-companion) then found 7 more issues the async loop missed (1 Critical + 4 High + 1 Medium + 1 Minor) — all fixed in `e21459c` before merge. Key changes from dual review: auto-commit route REMOVED from Sentry Layer D (every fix terminates in PR via Mode 1); Mode 2 constrained to runtime/operational work only (CLAUDE.md mandatory-skill rule unchanged); webhook dedupe rewritten with atomic claim before agent spawn; SENTRY_DAEMON_DSN provisioning claim corrected (Plan 01b CREDENTIALS array does NOT include Sentry entries yet — Layer A's PR must add them). Full audit trail at `.iago/reviews/2026-05-19-dual-pr71-aggressive/`. | [#71](https://github.com/ilsantino/iago-os/pull/71) → `784d210` |
| 2026-05-19 | merge | PR #70 — README rewrite reframing iaGO-OS as v2 multi-agent OS (multi-agent OS on VPS, Telegram-controlled, 5-shape AgentRuntime). Delivery pipeline now positioned as Layer 5 of 5. v1 skills/agents/hooks/Codex/MCP/memory sections preserved as collapsible details. | [#70](https://github.com/ilsantino/iago-os/pull/70) → `4e9724e` |
| 2026-05-19 | fast | Preserve 04a PR-triage artifacts summary from iago-os-c worktree before worktree cleanup. | `ffe4867` |
| 2026-05-19 | merge | PR #66 — preserve Phase 2 dual-review artifacts (Round 1+2). Includes Round 3 audit trail (Opus aggressive + Codex per PR) with @claude-bot sanitization pass (33 username redactions, PS trace strip, archive headers/footers). | [#66](https://github.com/ilsantino/iago-os/pull/66) → `c0fd713` |
| 2026-05-19 | merge | PR #68 — cutover/rollback executables + dry-run harness (Plan 03a). `cutover.sh` (T-15→T+60), `rollback.sh` (≤4-min OpenClaw restore + Telegram token rotation), `test-cutover.mjs` (23-case dry-run harness, no real VPS). Round 3 aggressive fixes for C-1 pipefail wrap at T+50, I-1A systemd-creds threshold, I-2A umask in rollback, I-3 T+05 retry. 23/23 tests passing. | [#68](https://github.com/ilsantino/iago-os/pull/68) → `8df36e8` |
| 2026-05-17 | plan | Feature plan + stress test for `feature-phase-2-vps-bootstrap` (FAST single-hour cutover: daemon deploy infra + OpenClaw teardown + cutover/rollback orchestration + first-real-workflow PR-triage agent + Phase 2 acceptance gate). L2 stage contract `CONTEXT.md` written per MWP discipline. Plans derive verbatim from `.iago/research/2026-05-16-phase-2-vps-bootstrap-spec.md` (10-section Garry-impressed delivery spec). | 7 plans / ~42 tasks / 3 waves (Plan 01 + Plan 04 at 8-task ceiling; Plan 06 + Plan 07 added during pre-merge adversarial-review fix wave per `.iago/reviews/adv-pr47-opus-20260516-204111.md` C3 + C4 fixes) — all 5 original stress verdicts PROCEED_WITH_NOTES. Critical findings forwarded inline: runUnder=test override for Phase 1 test compat (P01-C1), credential-value never in telemetry (P01-C2), jq pre-flight + interactive-flag matrix for dry-run + age-header magic-byte check (P02-C1/C2/C3), cutover DRY_RUN flag matrix + quote-escaping via scp temp file + rollback DRY_RUN (P03-C1/C2/C3), Task 1 fork explicit + gh-token CRED_MAP authored in Plan 01 + exact PAT scope (P04-C1/C2/C3), VPS_E2E_NONDISRUPTIVE mode + HTML-comment sentinel + fenced-only path check (P05-C1/C2/C3). Plan 06 (SIGHUP credential-reload, migration-scope § 13.3 gap closure) + Plan 07 (cron-scheduler subsystem, carved out of P04 Task 7 scope creep) stress-tested inline at creation; 3 spec gap items explicitly deferred to Phase 3+ per Plan 06 rationale in CONTEXT.md (TRIGGER-WATCH.md, persistent Tailscale ACL, cron-migration inventory). |
