...
2. **Bulk renaming inside OneDrive is not free — and it can be silently undone.** A rename re-uploads the file and can drop its version history. Worse: on 2026-08-19 a bulk move of 1,057 screenshots was followed immediately by a bulk rename, the lint reported **1,061/1,061 conforming**, and a later check found **224 of those renames reverted to their original names** — the sync client reconciling files it was still uploading and restoring the server-side name. No duplicates, no error, no journal entry: the rename simply came back. Let sync settle between a bulk move and a bulk rename, and **always re-lint some minutes later**, because the failure mode is a green light. Re-running the rename fixed all 224.
