...
| Bash deploy script lacking `set -euo pipefail` at top (or equivalent `set -e; set -u; set -o pipefail`) — undefined-variable typos and pipeline-stage failures silently continue. Exclusion: sourced library files (invoked via `.` or `source`, not direct entry points) inherit the sourcing script's shell options and are exempt from this floor. | ALWAYS Critical |
...
- **Remote pipe pipefail.** Every ssh-borne pipeline that the local script branches on must wrap the remote side in `bash -c 'set -o pipefail; CMD'`. The local shell's `set -o pipefail` is NOT inherited across ssh — the remote `sh -c` (default ssh exec shell) does not set pipefail. Without it: `journalctl | grep -E pattern` returns the exit code of grep (the last command), not journalctl — so a journalctl failure (exit 1) is hidden when grep matches at least one line (exit 0).
