...
| TanStack Query (or SWR) key for a multi-tenant aggregate omits `organizationId` / `tenantId` / `userId` — cache bleeds across tenants on switch / re-login — only fires on a model whose schema includes an `organizationId` / `tenantId` / `accountId` field | ALWAYS Critical |
