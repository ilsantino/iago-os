...
| Multi-tenant aggregate (count, sum, list feeding a KPI) built from a query that lacks an explicit `organizationId` / `tenantId` filter and relies solely on AppSync/auth rules | ALWAYS Critical |
| Aggregate / KPI / chart computed from a paginated list without a `nextToken` / cursor loop that fetches every page — the visible total is the page size, not the truth | ALWAYS Important |
...
| TanStack Query (or SWR) key for a multi-tenant aggregate omits `organizationId` / `tenantId` / `userId` — cache bleeds across tenants on switch / re-login | ALWAYS Important |
...
- **Multi-tenant aggregate without explicit tenant filter.** Any list/count/sum query for a multi-tenant product must pass `organizationId` / `tenantId` as a query filter — relying on AppSync auth rules alone breaks the moment a group is widened or the user is added to an admin role. Aggregate must reflect the user's tenant scope, not the auth scope.
- **Cache key omits tenant / user ID.** TanStack Query / SWR keys for tenant-scoped aggregates must include the tenant identifier in the key array. Without it, switching tenants or re-logging shows stale numbers from the previous context.
